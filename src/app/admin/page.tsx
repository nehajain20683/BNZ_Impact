'use client';
// src/app/admin/page.tsx — CLIENT COMPONENT (safer - errors shown in UI not silent redirect)
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <button onClick={loadDashboard} className="bg-sage-700 text-white px-4 py-2 rounded-xl text-sm">
            Retry
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-forest-950 text-white px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 text-sm text-forest-400">
          {role === 'SUPER_ADMIN' && (
            <Link href="/superadmin" className="hover:text-white text-xs bg-purple-600 px-3 py-1 rounded-lg text-white">
              ⚡ Superadmin
            </Link>
          )}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { icon: Activity,  label:'Farmers Assigned', value:(s.assignmentCount||0).toLocaleString('en-IN'),bg:'bg-indigo-500' },
          ].map(({ icon: Icon, label, value, bg }) => (
            <div key={label} className="bg-white rounded-xl border p-5 shadow-sm">
              <div className={`${bg} w-10 h-10 rounded-lg flex items-center justify-center mb-3`}>
                <Icon className="w-5 h-5 text-white"/>
          ...
          <div className="bg-white rounded-xl border p-5 shadow-sm mb-8">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-gray-500 text-sm">
                {(s.treesPlanted||0).toLocaleString('en-IN')} / {(s.plannedTrees||0).toLocaleString('en-IN')} trees
              </span>
            </div>
            <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-sage-500 to-green-500 rounded-full"
                style={{ width: `${Math.min(100, ((s.treesPlanted||0) / (s.plannedTrees||1)) * 100)}%` }}/>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          {[
            ...
            ...(role === 'SUPER_ADMIN' ? [{ href:'/superadmin', icon:'⚙️', label:'Superadmin', desc:'Manage organisations', color:'bg-purple-50 text-purple-700 border-purple-100' }] : []),
          ].map(l => (
          ...
          <div className="bg-white border rounded-xl overflow-hidden shadow-sm">
            ...
            {recentSites.length === 0 ? (
              <div className="text-center py-8">
                ...
                <Link href="/admin/plantation-sites" className="text-sage-600 text-xs hover:underline">Create first site →</Link>
              </div>
            ) : (
                ...
                {recentSites.map((site: any) => {
                  ...
                  return (
                    <Link key={site.id} href={`/admin/plantation-sites/${site.id}`}
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${PHASE_COLORS[site.currentPhase]||'bg-gray-100 text-gray-600'}`}>
                ...
                <div key={d.id} className="flex items-center gap-3 px-5 py-3">
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                      d.paymentStatus==='COMPLETED'?'bg-green-100 text-green-700':'bg-amber-100 text-amber-700'}`}>
